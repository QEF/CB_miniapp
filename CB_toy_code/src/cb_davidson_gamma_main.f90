   program cb_davidson_gamma_main
...
!--- THIS IS THE RELEVANT CALL TO THE ROUTINE IN KS_Solvers/Davidson ------------------------------------------!
     ...
     if (ndiag == 1) then
#endif
        call regterg( cb_h_psi, cb_s_psi, overlap, cb_g_psi, &
                      npw, npwx, nbnd, nbndx, evc, ethr, &
                      eig, btype, notcnv, lrot, dav_iter, nhpsi )
#if defined(__MPI)
     else
        call pregterg(cb_h_psi, cb_s_psi, overlap, cb_g_psi, &
                      npw, npwx, nbnd, nbndx, evc, ethr, &
                      eig, btype, notcnv, lrot, dav_iter, nhpsi )
     end if
#endif
...
! 
  ...
  call print_clock('ffts')
   ...
   end program cb_davidson_gamma_main
